...
2026 AMET은 한국자율주행산업협회가 안내한 자율주행 해커톤으로, 2026년 8월 25일부터 27일까지 코엑스에서 열린 자율주행모빌리티산업전 기간에 진행됐습니다. 이 저장소는 대회 순위나 주행 성과를 별도로 주장하지 않고, 공개된 코드와 실행 경로를 설명합니다.
...
센서 API가 제공되는 시뮬레이션 환경에서 먼저 확인합니다.
...
기본 설정은 최대 속도 0.80 m/s, 최소 속도 0.40 m/s이며 환경변수로 변경할 수 있습니다. 실제 주행 전에는 충분한 안전 공간과 즉시 정지할 수 있는 수단을 확보해야 합니다.
...
- 실제 차량 실행은 물리적 충돌 위험이 있습니다. 먼저 저속·짧은 시간으로 검증하십시오.
- 이 저장소에는 별도 라이선스 파일이 없습니다. 재사용 전 코드 및 외부 모델의 이용 조건을 확인해야 합니다.
